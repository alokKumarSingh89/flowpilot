...
Persist Execution Tool Spans Retrieval Metadata
...
RUNTIME-006, RUNTIME-007, OBS-002, OBS-003, OBS-006

# Objective

Persist execution/tool spans, retrieval metadata, termination/error class, token and estimated-cost measures with redaction.
...
- Implement only: Persist execution/tool spans, retrieval metadata, termination/error class, token and estimated-cost measures with redaction.
...
Update the approved ai telemetry area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the ai telemetry contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.

# Data Changes

Add or update only the minimal ai telemetry data representation needed for the cited requirements, with workspace ownership where applicable.
...
- Integration: cover the changed ai telemetry boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Every execution has trace ID and sanitized operational record.
