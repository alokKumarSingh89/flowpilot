...
Implement Provider Neutral Chat Embedding Gateway
...
RUNTIME-001, RUNTIME-006, RUNTIME-007, SEC-008, OBS-002

# Objective

Implement provider-neutral chat/embedding gateway boundary, safe request shaping, and AI trace creation.
...
- Implement only: Implement provider-neutral chat/embedding gateway boundary, safe request shaping, and AI trace creation.
...
Update the approved provider gateway area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the provider gateway contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.

# Data Changes

Add or update only the minimal provider gateway data representation needed for the cited requirements, with workspace ownership where applicable.
...
- Integration: cover the changed provider gateway boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Provider calls have sanitized traces and no direct domain-provider coupling.
