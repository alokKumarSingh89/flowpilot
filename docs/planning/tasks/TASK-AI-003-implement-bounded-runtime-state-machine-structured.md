...
Implement Bounded Runtime State Machine Structured
...
RUNTIME-004, RUNTIME-005, RUNTIME-008, RUNTIME-009, SEC-005, SEC-006

# Objective

Implement bounded runtime state machine, structured tool proposal validation, grant checks, and loop/limit enforcement.
...
- Implement only: Implement bounded runtime state machine, structured tool proposal validation, grant checks, and loop/limit enforcement.
...
Yes. After its dependencies are complete, it can run in parallel with: TASK-AGENT-001, TASK-AGENT-002, TASK-AGENT-004, TASK-AGENT-005.

# Expected Changes

Update the approved execution area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the execution contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.

# Data Changes

Add or update only the minimal execution data representation needed for the cited requirements, with workspace ownership where applicable.

# Security Requirements

The model proposes but never authorizes tools. Enforce grants, schemas, timeouts, limits, and loop prevention.

# Observability Requirements

Record proposed, denied, and executed tool spans plus bounded-execution termination reason; redact arguments/content as required.

# Testing Requirements

- Unit: cover task-specific validation, state, policy, and failure rules.
- Integration: cover the changed execution boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Model cannot self-authorize; tool/turn/time/token limits terminate safely.
