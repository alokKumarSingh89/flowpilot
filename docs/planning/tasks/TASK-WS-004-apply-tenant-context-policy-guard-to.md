...
Apply Tenant Context Policy Guard To
...
WS-008, RBAC-003, SEC-001, SEC-002

# Objective

Apply tenant context/policy guard to workspace-resource routes and repositories.
...
- Implement only: Apply tenant context/policy guard to workspace-resource routes and repositories.
...
Update the approved tenancy area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the tenancy contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.

# Data Changes

Add or update only the minimal tenancy data representation needed for the cited requirements, with workspace ownership where applicable.
...
- Integration: cover the changed tenancy boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Cross-workspace resources return no data and cannot mutate.
