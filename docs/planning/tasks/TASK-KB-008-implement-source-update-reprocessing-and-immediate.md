...
Implement Source Update Reprocessing And Immediate
...
KB-009, KB-010, JOB-001, JOB-002, JOB-003, JOB-004

# Objective

Implement source update/reprocessing and immediate retrieval disable plus asynchronous deletion cleanup.
...
- Implement only: Implement source update/reprocessing and immediate retrieval disable plus asynchronous deletion cleanup.
...
Yes. After its dependencies are complete, it can run in parallel with: TASK-KB-002, TASK-KB-003, TASK-KB-004, TASK-KB-005.

# Expected Changes

Update the approved lifecycle area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the lifecycle contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.

# Data Changes

Add or update only the minimal lifecycle data representation needed for the cited requirements, with workspace ownership where applicable.
...
- Integration: cover the changed lifecycle boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Updated source serves last ready version until replacement; deleted source is not retrievable.
