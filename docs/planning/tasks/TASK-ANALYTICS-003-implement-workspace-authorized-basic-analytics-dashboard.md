...
Implement Workspace Authorized Basic Analytics Dashboard
...
ANALYTICS-001, ANALYTICS-002, ANALYTICS-003, ANALYTICS-004, ANALYTICS-005, RBAC-005
...
- Implement only: Implement workspace-authorized basic analytics dashboard.
...
Yes. After its dependencies are complete, it can run in parallel with: TASK-ANALYTICS-001, TASK-OPS-001, TASK-OPS-002, TASK-OPS-003.

# Expected Changes

Update the approved analytics ui area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the analytics ui contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.

# Data Changes

Add or update only the minimal analytics ui data representation needed for the cited requirements, with workspace ownership where applicable.
...
- Integration: cover the changed analytics ui boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Authorized users see only their workspace’s metrics.
