...
Apply Role Aware Navigation Actions While
...
Apply role-aware navigation/actions while retaining server enforcement.
...
- Implement only: Apply role-aware navigation/actions while retaining server enforcement.
...
Update the approved frontend access area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the frontend access contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.
...
- Integration: cover the changed frontend access boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Users see only permitted actions; read-only views cannot submit writes.
