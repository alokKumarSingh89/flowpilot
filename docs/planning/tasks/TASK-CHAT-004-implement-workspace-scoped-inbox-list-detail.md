...
Implement Workspace Scoped Inbox List Detail
...
CHAT-007, HANDOFF-004, HANDOFF-005, HANDOFF-006, RBAC-002

# Objective

Implement workspace-scoped inbox/list/detail API and role-authorized support views.
...
- Implement only: Implement workspace-scoped inbox/list/detail API and role-authorized support views.
...
Yes. After its dependencies are complete, it can run in parallel with: TASK-CHAT-001, TASK-CHAT-003, TASK-HANDOFF-001, TASK-HANDOFF-002.

# Expected Changes

Update the approved support inbox area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.

# API / Contract Changes

Define or update only the support inbox contract required by this task. Document validation, authorization, and failure behavior; do not add unrelated endpoints.

# Data Changes

Add or update only the minimal support inbox data representation needed for the cited requirements, with workspace ownership where applicable.
...
- Integration: cover the changed support inbox boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Support users view only permitted workspace conversations.
