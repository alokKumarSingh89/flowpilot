...
Establish Durable Job Envelope Idempotency Key
...
JOB-001, JOB-002, JOB-003, JOB-004, OBS-004

# Objective

Establish durable job envelope, idempotency key, bounded retry/DLQ policy, and worker correlation propagation.
...
- Implement only: Establish durable job envelope, idempotency key, bounded retry/DLQ policy, and worker correlation propagation.
...
Yes. After its dependencies are complete, it can run in parallel with: TASK-PLAT-002, TASK-PLAT-003, TASK-PLAT-005, TASK-PLAT-006.

# Expected Changes

Update the approved jobs area, focused interfaces/contracts, tests, and necessary configuration/documentation. Use the structure established by prerequisite tasks; do not invent unrelated application paths or packages.
...
- Integration: cover the changed jobs boundary, persistence, queue, storage, provider, or API path as applicable.
...
- [ ] Duplicate jobs are safe; retry exhaustion is visible and routed to DLQ.
